...
class GetReportPosition:
    ...
    def get_results_statistics_position(self, data_dict):
        ...
        x_lux_positions = [data_dict[k][0] for k in data_dict.keys()]
        x_max = max(x_lux_positions)
        # y_max = data_dict["lux_1000"][1]
        y_max = max([data_dict[k][1] for k in data_dict.keys()])
        ...
        elif os.listdir(Config.ae_400lux_frames_path):
            lx_50_result_position = [x_max + 1, y_max - 1]
            lx_400_result_position = [x_max + 1, y_max]
            frame_que_position = [x_max + 1, y_max - 2]
        elif os.listdir(Config.ae_1000lux_frames_path):
            lx_50_result_position = [x_max + 1, y_max]
            frame_que_position = [x_max + 1, y_max - 1]

        # 最后一行的数据
        result_dict = {}
        result_dict["calculate_result"] = frame_que_position
        if os.listdir(Config.ae_50lux_frames_path):
            result_dict["lx_50_frames_sum"] = lx_50_result_position
        if os.listdir(Config.ae_400lux_frames_path):
            result_dict["lx_400_frames_sum"] = lx_400_result_position
        if os.listdir(Config.ae_1000lux_frames_path):
            result_dict["lx_1000_frames_sum"] = lx_1000_result_position
